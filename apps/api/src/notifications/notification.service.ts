...
function alertBanner(icon: string, message: string, bg: string, border: string, color: string): string {
...
function infoBox(content: string, borderColor = '#3b82f6', bg = '#f1f5f9'): string {
...
export class NotificationService {
  ...
  async notifyPaymentVerified(
    ...
    const ctaUrl = isVendor ? `${webUrl}/vendor/handover` : `${webUrl}/client/handover`;
  ...
  async notifyVendorPickupRequested(
    ...
    return this.sendEmail({
      ...
      body: emailLayout(`
        ...
        ${dataTable([
          ['Name', userName],
          ['Email', `<a href="mailto:${userEmail}" style="color:#3b82f6;font-weight:700;">${userEmail}</a>`],
          ['Phone', userPhone ? `<a href="tel:${userPhone}" style="color:#3b82f6;">${userPhone}</a>` : 'Not provided'],
        ])}
    ...
    return this.sendEmail({
      ...
      body: emailLayout(`
        ...
        ${dataTable([
          ['Auction', auctionTitle],
          ['Client', clientName],
          ['Gate Pass No.', `<strong style="color:#166534;font-size:15px;">${gatePassNumber}</strong>`],
        ])}
  ...
  async notifyAdminNewRegistration(
    ...
    return this.sendEmail({
      ...
      body: emailLayout(`
        ...
        ${dataTable([
          ['Name', userName],
          ['Email', `<a href="mailto:${userEmail}" style="color:#3b82f6;">${userEmail}</a>`],
          ['Role', role],
          ['Company', companyName],
          ['Registered On', registrationDate.toLocaleString('en-IN', { dateStyle: 'long', timeStyle: 'short' })],
        ])}
    ...
    return this.sendEmail({
      ...
      body: emailLayout(`
        ...
        ${dataTable([
          ...
          ['SPOC Name', spocName],
          ['SPOC Phone', `<a href="tel:${spocPhone}" style="color:#3b82f6;font-weight:700;">${spocPhone}</a>`],
        ])}
    ...
    return this.sendEmail({
      ...
      body: emailLayout(`
        ...
        ${dataTable([
          ...
          [`Pay to Client (${clientName})`, `<strong style="color:#166534;">INR ${clientAmount.toLocaleString('en-IN')}</strong>`],
          ['WeConnect Commission (5%)', `<strong style="color:#1e40af;">INR ${commissionAmount.toLocaleString('en-IN')}</strong>`],
        ])}
        ${divider}
