      ...
      {isOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden dark:bg-slate-900"
          >
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between dark:border-slate-800">
              <h3 className="text-xl font-black text-slate-900 tracking-tight dark:text-white">{title}</h3>
              <button onClick={onClose} className="w-8 h-8 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-slate-100 transition-colors dark:bg-slate-950">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {itemDetails.map((detail, idx) => (
                  <div key={idx} className="bg-slate-50 p-4 rounded-2xl dark:bg-slate-950">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{detail.label}</p>
                    <p className="text-sm font-bold text-slate-900 truncate dark:text-white">{detail.value}</p>
                  </div>
                ))}
              </div>
