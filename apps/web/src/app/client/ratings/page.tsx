...
export default function ClientRatingsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const completed = (res.data ?? []).filter((p: any) =>
        p.status === "COMPLETED" && p.auction?.client?.id === currentUser.companyId
      );
  ...
  const submitRating = async (pickup: any) => {
    ...
    try {
      await api.post("/ratings", {
        ...
        toCompanyId: pickup.auction?.winner?.id,
        score: form.score,
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black text-slate-900 dark:text-white">Rate Vendors</h1>
        <p className="text-sm text-slate-500 mt-1">Share your feedback on vendors who completed your e-waste pickup.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-slate-400">
          ...
          <p className="text-sm">Rating options appear once a pickup is marked completed.</p>
        </div>
      ) : (
              ...
              <div key={pickup.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">Vendor: <span className="font-bold">{pickup.auction?.winner?.name}</span></p>
                  </div>
                  {isSubmitted && (
                    <div className="flex items-center gap-1">
                      {[1,2,3,4,5].map(s => (
                        <span key={s} className={`text-sm ${s <= existing.score ? "text-amber-400" : "text-slate-300"}`}>★</span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <textarea rows={2} value={form.comment}
                      ...
                      placeholder="How was the vendor's service and compliance?"
