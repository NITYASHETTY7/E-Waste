...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const mapRequirementToListing = (req: any): Listing => {
    ...
    return {
      ...
      auctionPhase,
      status: req.auction?.status === 'COMPLETED' ? 'completed' : (req.status === 'FINALIZED' ? 'active' : 'pending'),
  ...
  const updateUserProfile = async (updates: Partial<User>) => {
    try {
      ...
      // Re-fetch profile to get fully updated and mapped user object (including company fields)
      const profileRes = await api.get('/auth/profile');
      const updatedUser = mapBackendUser(profileRes.data);
  ...
  const changePassword = async (newPassword: string, oldPassword: string) => {
    ...
    setState(prev => {
      ...
      return {
        ...prev,
        users: prev.users.map(u => u.id === prev.currentUser?.id ? { ...u, password: newPassword } : u),
      };
    });
  };
