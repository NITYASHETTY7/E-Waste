...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await login("admin", loginEmail, loginPassword);
      if (user.role !== "admin") {
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:bg-white dark:focus:bg-slate-800 transition-all font-medium dark:bg-slate-950 dark:border-slate-700"
            />
          </div>
          <div>
            <label className="text-[10px] uppercase font-black tracking-widest text-slate-500 mb-1 block">Security Key / Password</label>
            <div className="relative">
              <input
                ...
                placeholder="••••••••"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:bg-white dark:focus:bg-slate-800 transition-all font-mono pr-12 dark:bg-slate-950 dark:border-slate-700"
              />
