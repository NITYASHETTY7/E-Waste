    ...
    <div className="dashboard-container space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      >
        <div>
          ...
          <p className="text-slate-500 font-medium">Managing disposal for <span className="text-slate-900 font-bold">{currentUser?.name}</span></p>
        </div>
        <div className="flex gap-3">
          <Link href="/client/post" className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-4">Active Lot Status</h3>
            <div className="space-y-4">
              {activeListings.slice(0, 3).map(listing => (
                <div key={listing.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500 uppercase font-black">{listing.auctionPhase?.replace('_', ' ')}</p>
                  </div>
                  <Link href={`/client/listings/${listing.id}`} className="text-emerald-600 hover:text-emerald-700">
                    <span className="material-symbols-outlined">arrow_forward_ios</span>
                  </Link>
                </div>
              ))}
              {activeListings.length === 0 && (
                <div className="text-center py-6 text-slate-400 text-sm italic">No active lots</div>
              )}
            </div>
          </div>
        </div>
