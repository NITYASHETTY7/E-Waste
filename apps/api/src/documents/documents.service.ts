...
<footer>Tax Invoice generated by WeConnect E-Waste Aggregator Platform. Auction Ref: ${p.auctionId}</footer>
  ...
  async executeGenerateWorkOrderPdf(payload: any): Promise<string> {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Work Order - <%= String(auctionId).substring(0, 8).toUpperCase() %></title>
        <style>
          body { font-family: Arial, sans-serif; padding: 40px; color: #111; font-size: 13px; }
          h1 { font-size: 20px; color: #1E8E3E; margin: 0; }
          .header { display: flex; justify-content: space-between; border-bottom: 2px solid #1E8E3E; padding-bottom: 16px; margin-bottom: 24px; }
    ...
    <body>
        <div class="header">
          <div>
            ...
            <p style="margin: 4px 0 0; color: #6b7280; font-size: 12px">WeConnect E-Waste Aggregator Platform</p>
          </div>
          <div style="text-align: right;">
            <p style="margin: 0; font-weight: bold; font-size: 16px">WO-<%= String(auctionId).substring(0, 8).toUpperCase() %></p>
            <p style="margin: 4px 0; font-size: 12px; color: #6b7280">Date: <%= new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) %></p>
            <span class="badge">OFFICIAL DOCUMENT</span>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col section">
            <div class="section-title">Client (Generator)</div>
            <p style="margin: 4px 0; font-weight: bold"><%= clientName %></p>
            <p style="margin: 2px 0; color: #374151">Registered Client</p>
          </div>
          <div class="col section">
            <div class="section-title">Vendor (Recycler)</div>
            <p style="margin: 4px 0; font-weight: bold"><%= vendorName %></p>
            <p style="margin: 2px 0; color: #374151"><%= vendorAddress || 'Address on file' %></p>
          </div>
        </div>
        ...
        <div class="sig">
            <div class="sig-box">Authorized Signatory (Client)</div>
            <div class="sig-box">Authorized Signatory (Vendor)</div>
        </div>
